...
/*  //return index of target, or -1 if not found (iterative method for binSearch)
  public static int binSearch(ArrayList al, int target)
  {
	  ...
	  int middle = ((start + end)/2);
		...
		while (start <= end)
		{
			if ((int)al.get(middle)==target)
				return middle;
			else if ((int)al.get(middle) > target) // Search through the lower half when middle value is greater than target value
			{
				end = middle -1;
			...
			{
				start = middle + 1;
				middle = ((middle + end)/2); // Search through the upper half when middle value is less than target value
			}
		}
	 ...
	 //Helper method for binSearch
	 public static int binSearch(ArrayList al, int target)
	 {			  
		return binSearch (al, 0, al.size()-1, target);
	 }
		  
	 // Recursive way of binSearch
	 public static int binSearch (ArrayList al, int start, int end, int target)
	 {
		 int middle = ((start + end)/2);
		 ...
		 {
			 if ((int)al.get(middle) == target) 
		 return middle; // return index when al[position] == target
			 else if ((int)al.get(middle) > target)
			 {
				 return binSearch(al, 1, middle -1, target); // Search through the lower half when middle value is greater than target value 
			 }
			 else 
				 return binSearch(al, middle + 1, end, target); //Search through the upper half when middle value is less than target value 
		 }
